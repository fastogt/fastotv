...
#include "client/player/media/app_options.h"  // for ComplexOptions, AppOpt...
#include "client/player/media/av_utils.h"
...
#include "client/player/media/frames/frame_queue.h"  // for VideoDecoder, AudioDec...
...
int VideoState::ReadRoutine() {
  ...
  if (!ic) {
    ...
    common::ErrnoError err = common::make_errno_error(AVUNERROR(av_errno), common::ERROR_TYPE);
    if (handler_) {
  ...
  if (uri_str.empty()) {
    common::Error err = common::make_error_inval(common::ERROR_TYPE);
    if (handler_) {
  ...
  if (open_result < 0) {
    ...
    common::Error err = common::make_error(err_str, common::ERROR_TYPE);
